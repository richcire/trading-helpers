...
function ReportDetail({accession,section}:{accession:string;section:string}){
 ...
 <p className="ww-note">{c.positionsShown} · {fmt(filtered.length)} / {fmt(events.length)}</p><div className="ww-event-list">{filtered.slice(0,limit).map(eventRow)}</div>{!filtered.length&&(events.length>0||hasPrevious)&&<p className="ww-empty">{events.length?c.empty:c.noChanges}</p>}{limit<filtered.length&&<button className="ww-button ww-load" onClick={()=>setLimit(n=>n+50)}>{c.loadMore}</button>}
 </>}
